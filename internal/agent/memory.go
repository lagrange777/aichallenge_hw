...
When the assistant presents or revises a concrete plan during planning, always include taskProposal with that plan in progress.plan, even when the proposed stage remains planning while awaiting approval. Keep the actual steps and verification criteria, not merely a reference to the chat answer. This is an unapproved draft, never a claim that the user accepted it.
Choose the proposed stage for the NEXT currentStep, not for the work just finished. After a plan is presented and implementation is next, propose execution; the user must explicitly approve the concrete plan via approve_plan; generic proposal acceptance cannot change stages. Include the actual plan in progress.plan. When validation finds a defect, propose execution with the repair as currentStep and put the findings in validation. The server resets the validation approval on return to execution. Never label implementation work as planning or repair work as validation.
Allowed stages: planning -> execution -> validation -> done; validation -> execution for fixes. Staying in the current stage is allowed. Never skip stages. Never perform work ahead of the confirmed stage. execution/validation -> planning is allowed only for explicit replanning. Proposals do not set approval metadata or validation status.
Limits in characters: plan 6000, goal 2000, currentStep 500, expectedAction 2000, completed 6000, result 6000, openQuestions 2000, validation 4000, reason 1000. goal/currentStep/expectedAction must be nonempty; expectedActor is agent or user.
...
func (a *Agent) proposeMemories(ctx context.Context, model, user, answer string, state memory.State) ([]memory.Proposal, *memory.TaskProposal, []memory.Invariant, Usage, *float64, string) {
	...
	if p := parsed.TaskProposal; p != nil {
		p.Progress = memory.PreserveApprovedArtifacts(state.Task.Workflow, p.Progress)
