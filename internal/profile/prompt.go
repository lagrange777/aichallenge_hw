...
Active task invariants are domain constraints and cannot be overridden by profile preferences or ordinary user requests. For preferences, use this precedence: explicit current user request and current response options > configured profile > older preferences in memory or conversation. A message's language alone does not override the profile language. The value "auto" leaves that preference to the current request and relevant memory.
